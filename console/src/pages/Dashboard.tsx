...
export function Dashboard() {
  ...
  const getColorClasses = (color: string) => {
    const colors: Record<string, { bg: string; text: string; icon: string }> = {
      blue: { bg: 'bg-blue-900/20', text: 'text-blue-400', icon: 'text-blue-600' },
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-gray-400 mt-1">Overview of your Sinas AI platform</p>
      </div>
      ...
      {showOnboarding && (
        <div className="bg-blue-900/20 border border-blue-800/30 rounded-lg p-5">
          <div className="flex items-start justify-between mb-3">
            <div>
              <h3 className="text-sm font-semibold text-blue-300">Get Started with SINAS</h3>
              <p className="text-xs text-blue-400 mt-0.5">Complete these steps to start using your AI platform</p>
            </div>
            <button
          ...
          <div className="space-y-2">
            ...
            >
              ...
              <span className={`text-sm flex-1 ${hasProviders ? 'text-gray-500 line-through' : 'text-gray-100 font-medium'}`}>
                Configure an LLM Provider
              </span>
              {!hasProviders && <ArrowRight className="w-4 h-4 text-gray-500 group-hover:text-blue-500" />}
            </Link>
            <Link
              to="/agents"
              className="flex items-center gap-3 p-2.5 bg-[#161616] rounded-md border border-blue-100 hover:border-blue-300 transition-colors group"
            >
              ...
              <span className={`text-sm flex-1 ${hasAgents ? 'text-gray-500 line-through' : 'text-gray-100 font-medium'}`}>
                Create an Agent
              </span>
              {!hasAgents && <ArrowRight className="w-4 h-4 text-gray-500 group-hover:text-blue-500" />}
            </Link>
            <Link
              to="/agents"
              className="flex items-center gap-3 p-2.5 bg-[#161616] rounded-md border border-blue-100 hover:border-blue-300 transition-colors group"
            >
              ...
              <span className={`text-sm flex-1 ${hasChats ? 'text-gray-500 line-through' : 'text-gray-100 font-medium'}`}>
                Start a Conversation
              </span>
              {!hasChats && <ArrowRight className="w-4 h-4 text-gray-500 group-hover:text-blue-500" />}
            </Link>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="card">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-semibold text-gray-100">Activity (Last 7 Days)</h3>
        ...
        {/* Role Distribution */}
        <div className="card">
          <h3 className="text-lg font-semibold text-gray-100 mb-4">Message Types</h3>
          {roleData.length > 0 ? (
              ...
              <div key={item.agent} className="flex items-center justify-between">
                <div className="flex items-center gap-3 flex-1">
                  ...
                  <Link
                    ...
                    className="text-sm font-mono text-gray-100 hover:text-primary-600"
                  >
                    {item.agent}
                  </Link>
                </div>
                <div className="flex items-center gap-2">
                  <span className="text-sm font-semibold text-gray-100">{item.count}</span>
